...
export const CommunityProvider = ({
  ...
  useEffect(() => {
    ...
    if (error?.type === LoadingErrorType.Map || !loading || loadingUrlParams)
      return
    ...
    async function loadData() {
      ...
      if (categories.current.length > 0 && !fetchingPlaces) {
        ...
        if (urlParams.placeSlug) {
          current = allPlaces.current.find(
            (place: Place) => place.slug === urlParams.placeSlug
          )
  ...
  if (error) {
    return <LoadingMap message={error.message} type={LoadingMapType.Error} />
  }
