...
const LegalCheck = ({ error, checked, onCheck }) => {
  ...
  const errorMessage = intl.formatMessage({
    defaultMessage: 'Debes aceptar los términos de legales',
    id: '3vFpuC'
  })
  const privacyLinkText = intl.formatMessage({
    defaultMessage: 'Política de privacidad',
    id: 'l4IlHe'
  })
  const cookiesLinkText = intl.formatMessage({
        ...
        <span className={styles.checkbox.label}>
          {legal.privacyLink && legal.cookiesLink
            ? intl.formatMessage(
                {
                  id: 'mFDtrP',
                  defaultMessage: 'Acepto la {privacyLink} y la {cookiesLink}'
                },
                {
                  privacyLink: (
                    <LegalLink
                      href={legal.privacyLink}
                      text={privacyLinkText}
                    />
                  ),
                  cookiesLink: (
                    <LegalLink
                      href={legal.cookiesLink}
                      text={cookiesLinkText}
                    />
                  )
                }
              )
