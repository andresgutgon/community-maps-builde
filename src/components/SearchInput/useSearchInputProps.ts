...
const DEFAULT_INPUT_CLASSES =
  'flex-1 bg-transparent w-[170px] sm:w-[400px] border-none focus:outline-none focus:ring-0 py-1 sm:py-2 pl-1 placeholder-gray-500 placeholder-opacity-50'
export enum ResultsTopSpace {
  sm = 'sm',
  normal = 'normal'
}
export enum ResultsXSpace {
  normal = 'normal'
}
export type ResutsListProps = {
  top: ResultsTopSpace
...
export const useSearchInputProps = ({
  ...
  return {
    placeholder: `${intl.formatMessage({
      defaultMessage: 'Buscar una dirección',
      id: 'IVA/Y9'
    })}...`,
    buttonLabel: intl.formatMessage({ defaultMessage: 'Buscar', id: 'eOuNie' }),
    formClasses: cn('flex items-center justify-between', {
      'space-x-2': inputButtonSeparation
    }),
