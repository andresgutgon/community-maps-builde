...
const useStateLabels = (): Labels => {
  ...
  return useRef<Record<State, StateLabel>>({
    [State.all]: {
      ...
      description: intl.formatMessage({ defaultMessage: 'Todos los lugares sin filtrar', id: 'VvIgYE' })
    },
    [State.starting]: {
    ...
    [State.active]: {
      ...
      description: intl.formatMessage({ defaultMessage: 'Estos puntos están funcionando', id: 'J3iaDf' })
    }
  }).current
}
