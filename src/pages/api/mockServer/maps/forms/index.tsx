...
type FakeResponse = { status: number; submitResponse: { message?: null | string }}
const formSubmit = async (request: NextApiRequest, response: NextApiResponse) => {
  ...
  const serverResponse: FakeResponse = request.method === 'POST'
    ? (
      schrodingerSuccess
        ? { status: 200, submitResponse: { message: 'Hemos recibido tu solicitud. Gracias!' }}
        : { status: 422, submitResponse: { message: 'Hubo un error en el servidor' } }
    )
    : { status: 422, submitResponse: { message: 'Error in the server' }}
